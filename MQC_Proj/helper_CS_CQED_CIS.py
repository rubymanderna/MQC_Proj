...
def nuclear_gradient_finite_diff(mol, cqed_rhf, delta=1e-4):
    natm = mol.natm
    ...
    for i in range(natm):
        for j in range(3):
            ...
            cqed_rhf_plus = CQED_RHF(mol_plus, cqed_rhf.cavity_mode)
            e_plus = cqed_rhf_plus.kernel()
            
            mol_minus = mol.copy()
            mol_minus.set_geom_(coords_minus, unit='Bohr')
            cqed_rhf_minus = CQED_RHF(mol_minus, cqed_rhf.cavity_mode)
            e_minus = cqed_rhf_minus.kernel()
            
            grad[i, j] = (e_plus - e_minus) / (2 * delta)
...
if __name__ == "__main__":
    mol = gto.Mole()
    mol.atom = 'H 0 0 0; F 0 0 1.1'  # Example: HF molecule
    mol.basis = 'sto-3g'
    mol.build()
    ...
    # Create CQED-RHF object
    cqed_rhf = CQED_RHF(mol, cavity_mode)
    cqed_rhf.kernel()

    # Compute the nuclear gradient
    grad = nuclear_gradient_finite_diff(mol, cqed_rhf)
